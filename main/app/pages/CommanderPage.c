...
static void CommanderPage_draw() {
    ...
    // Selection frame rendering using interpolated coordinates
    if (commander_page.selected_label != NULL) {
        ...
        // Delta indicator for uncommitted changes
...
static void CommanderPage_on_tick(uint32_t delta_ms) {
    ...
    // Scan for pending commits across the grid
    for (int p = 0; p < 4; p++) {
        for (int s = 0; s < 4; s++) {
    ...
    // Auto-commit changes after 2 seconds of inactivity
    if (changes_pending) {
    ...
    if (commander_page.selected_label != NULL) {
        ...
        const float old_x = commander_page.anim_x;
        const float old_y = commander_page.anim_y;
...
static void CommanderPage_editor_callback(int32_t new_value) {
    ...
    const int32_t val_id = CommanderPage_get_value_id(s_id);
    const int32_t old_cmd_val = Game_get_value(p_id, val_id);
    const int32_t diff = new_value - old_cmd_val;

    Game_set_value(new_value, p_id, val_id);

    // Dynamic HP update based on value difference
    if (diff != 0) {
        int32_t current_hp = Game_get_value(p_id, INDEX_HP);
