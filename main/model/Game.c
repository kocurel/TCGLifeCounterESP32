...
/**
 * Loads game state from persistent settings
 */
void Game_init() {
    game.history.head = 0;
    ...
    /* Initialize labels and player structures from storage */
...
/**
 * Resets session values to defaults and clears history
 */
void Game_reset() {
    GameSettings settings = SettingsModel_get();
    game.number_of_players = settings.player_count;
...
/**
 * Ensures monarch status remains unique across all players
 */
...
/**
 * Appends a change to the circular history buffer and discards redo timeline
 */
static void History_add_change(ValueChange new_change) {
    ...
    /* Drop oldest event if buffer capacity is exceeded */
...
/**
 * Updates player value and logs the transition delta
 */
...
/**
 * Synchronizes state with a specific history index via multiple undo/redo steps
 */
...
/**
 * Returns a specific historical change using relative indexing
 */
