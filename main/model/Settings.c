...
/**
 * Populates the settings structure with default naming conventions.
 * Handles both generic placeholder names and game-specific defaults.
 */
static void set_default_value_names(GameSettings* settings) {
    // 1. Initialize all slots to generic "Val X"
    for (int i = 0; i < SETTINGS_VAL_COUNT; i++) {
        snprintf(settings->value_names[i], SETTINGS_VAL_NAME_LEN, "Val %d",
                 i + 1);
    }

    // 2. Assign Magic: The Gathering specific defaults
    ...
    // Default Commander Damage labels (indices 28-31)
...
/**
 * Initializes the NVS flash partition and loads saved settings into RAM.
 * If no saved settings exist, populates the model with defaults.
 */
void SettingsModel_init() {
    ...
    if (err != ESP_OK) {
        return;  // Use defaults if NVS cannot be opened
    }
    ...
    // Apply initial hardware state based on loaded settings
    GUIRenderer_set_contrast(s_current_settings.screen_brightness);
}

/**
 * Saves the entire settings structure to NVS Flash.
 */
...
/**
 * Updates and persists a specific player's name.
 */
...
/**
 * Updates and persists a specific value name (e.g., renaming "Poison" to
 * "Infect").
 */
