...
void test_Safety_Null_Inputs() {
    ...
    GUI_ADD_CHILD(real_vbox, null_comp);   // Valid Container, NULL Child
                                           // (assuming Lable is not container)
    ...
    // Note: If you were modifying a global state variable (like a screen buffer
...
void test_HBox_Full_Width_Distribution() {
    ...
    TEST_ASSERT_EQUAL_UINT8(10, l1->base.x);       // Start X (10) + Padding (0)
    TEST_ASSERT_EQUAL_UINT8(10, l1->base.y);       // Start Y (10) + Padding (0)
